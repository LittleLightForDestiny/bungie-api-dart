  ...
  if (typeRef && componentByDef[typeRef]) {
    if (typeRef.includes('/responses/')) {
      ...
      if (component) {
        ...
        if (property) {
          importFiles['common.dart'] = importFiles['common.dart'] || new Set();
          importFiles['common.dart'].add('ServerResponse');
...
export function generateImports(filename: string , importFiles: { [filename: string]: Set<string> }): string {
  return _.compact(_.map(importFiles, (types, f) => {
    const absImport = path.resolve('../lib', f);
    const absDest = path.resolve(filename);
    if (absImport === absDest) {
      return undefined;
    }
    let relativePath = path.relative(path.dirname(absDest), absImport).replace(/(\.d)?\.dart$/, '');
    if (!relativePath.startsWith('.')) {
      relativePath = './' + relativePath;
    }
    return `import '${relativePath}.dart';`;
  })).sort().join("\n");
}
