...
If False, they may not actually be visible in the world at the moment. */
	...
	static DestinyVendorBaseComponent fromMap(Map<String, dynamic> data){
		if(data == null) {
			return null;
		};
		return new DestinyVendorBaseComponent(
				data['vendorHash'],
	...
	static List<DestinyVendorBaseComponent> fromList(List<dynamic> data){
		if(data == null) {
			return null;
		};
		List<DestinyVendorBaseComponent> list = new List();
    data.forEach((item) {
      list.add(DestinyVendorBaseComponent.fromMap(item));
    });
    return list;
	}
