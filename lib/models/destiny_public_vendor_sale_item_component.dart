...
Note that there's not actually any guarantee that it will go away: it could be chosen again and end up still being in the Vendor's sale items! But this is the next date where that test will occur, and is also the date that the game shows for availability on things like Bounties being sold. So it's the best we can give. */
	...
	static DestinyPublicVendorSaleItemComponent fromMap(Map<String, dynamic> data){
		if(data == null) {
			return null;
		};
		return new DestinyPublicVendorSaleItemComponent(
	...
	static List<DestinyPublicVendorSaleItemComponent> fromList(List<dynamic> data){
		if(data == null) {
			return null;
		};
		List<DestinyPublicVendorSaleItemComponent> list = new List();
    data.forEach((item) {
      list.add(DestinyPublicVendorSaleItemComponent.fromMap(item));
    });
    return list;
	}
